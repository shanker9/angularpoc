...
export class AppComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    console.log('Total no. of strategies: ',this.names.length);
    for (let i = 1; i <= 30; i++) {
  ...
  ngAfterViewInit() {
    (document.querySelector('.cdk-virtual-scroll-content-wrapper') as HTMLElement).style.display = 'flex';
  }

  onScroll(e) {
    let elem = document.getElementById('structure_header');
    elem.scrollTop = e.target.scrollTop;
  }
}